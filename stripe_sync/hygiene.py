...
# Стадии, где риск - ФАКТ, а не догадка: платёж не прошёл, подписка отменена.
# Их считает живая вьюха по событиям, а скор p_churn пересчитывается раз в
# сутки. Если карта отвалилась в 10 утра, ночной скор про это ещё не знает - и
# гейт «дарим только рискующим» зарубил бы именно то касание, ради которого всё
# строилось.
#
# SAVE ЗДЕСЬ НЕТ, И ЭТО НАМЕРЕННО. «Признаки ухода» - это вывод по поведению
# (притих, заходил на страницу отмены), а не свершившийся факт. В замерах
# удерживающих кампаний 4-5% людей уходят ИМЕННО ПОТОМУ, что их потревожили:
# они бы остались, но подарок «мы заметили, что вы собираетесь уйти» напомнил
# им, что они платят. Такие сидят ровно среди спокойных, поэтому денежный
# подарок на SAVE обязан спрашивать скор.
RISK_STAGES = ("DUNNING", "WINBACK")

# ...но само касание на SAVE не ждёт ночного скора: бесплатные рычаги (письмо,
# пауза) уходят сразу. Порог стоит только на деньгах.
SOFT_RISK_STAGES = ("SAVE",)
