...
# пароль - только [A-Za-z0-9] (см. генерацию), прямая подстановка безопасна
docker exec "$DB" psql -U postgres -d postgres -c \
  "UPDATE auth.users SET encrypted_password = extensions.crypt('$NEWPW', extensions.gen_salt('bf'));" >/dev/null
