...
Use aggregated endpoints when:

* **High transaction volume** — Your RPS (requests per second) is too high for individual transaction uploads
* **Batch processing** — You prefer to aggregate data in your ETL pipeline before sending
* **Network efficiency** — Reducing number of API calls is a priority
* **Pre-computed metrics** — You already aggregate data for internal analytics

Use transaction-level endpoints when:

* Transaction volume is manageable (< 1000 transactions per minute)
* Your system doesn't have aggregation capabilities
...
| `POST /v1/data/payments-daily` | `POST /v1/data/payments` | Daily aggregated payment statistics |
...
```
POST /v1/data/payments-daily
```

### Grouping Dimensions

Aggregate your transaction-level payments by this unique combination:
...
* `action_sum` — total of all payment amounts
* `action_count` — number of payment transactions
...
SELECT 
  ...
  currency,
  is_real_operation,
  SUM(amount) AS action_sum,
  COUNT(*) AS action_count
FROM payments
WHERE DATE(payment_timestamp) = '2024-02-01'
GROUP BY date, user_id, action, status, country, currency, is_real_operation;
```
...
daily_payments = df.groupby([
    'date', 'user_id', 'action', 'status',
    'country', 'currency', 'is_real_operation'
]).agg(
    action_sum=('amount', 'sum'),
    action_count=('amount', 'count')
).reset_index()
```
