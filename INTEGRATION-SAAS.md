...
## 6. Каналы касаний: как это устроено

ИНФРАСТРУКТУРА каналов - наша (аккаунты Resend и DecisionTelecom, боты),
клиент не заводит аккаунтов у провайдеров. НО КАЖДОЕ КАСАНИЕ УХОДИТ ОТ ИМЕНИ
КЛИЕНТА - это зашито в платформу (per-tenant идентичность отправителя):

- **Email**: с поддомена клиента, например `care@mail.hubcontent.com`. Мы
  выдаём 3 DNS-записи (DKIM/SPF), клиент добавляет их к своему домену - вся
  почта подписана его доменом, в поле «От» его бренд. Прогрев домена ведём мы.
- **SMS/Viber**: отправитель - альфа-имя клиента («HubContent»), регистрируем
  на его бренд через наш аккаунт DecisionTelecom.
- **Telegram**: брендированный бот клиента (имя и аватар его продукта).
- **WhatsApp**: номер и верификация Meta принадлежат бизнесу клиента (WABA).

Адреса юзеров для email приходят из Stripe при склейке - канал работает из
коробки после DNS-настройки.
