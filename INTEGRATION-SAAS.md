...
Партнёрская модель: ИНФРАСТРУКТУРА каналов - наша (аккаунты Resend и
DecisionTelecom, доставка, вебхуки), клиент не заводит аккаунтов у провайдеров
и не платит им напрямую. НО КАЖДОЕ КАСАНИЕ УХОДИТ ОТ ИМЕНИ КЛИЕНТА -
per-tenant идентичность отправителя зашита в платформу.

Всё подключается САМИМ клиентом в CRM → **Channels** (/channel-settings):

**Email** (Resend, наш аккаунт):
...
3. Клиент задаёт «От кого»: имя + адрес строго на подключённом поддомене.
   ...
   (Если платформенный ключ Resend ещё не введён - запрос сохраняется со
   статусом awaiting_provider и досоздаётся автоматически при первом Check.)
...
3. Юзеров подключает диплинком `https://t.me/<бот>?start=<client_user_id>` -
   нажатие Start прилетает вебхуком и превращается в contact_update с
...
Правила отправки (зашиты в раннер): канал без контакта - касание не уходит
(reason `no_contact`); контакт без согласия - не уходит (`no_consent`);
US-номера - без SMS/звонков (TCPA). Кампания сама выбирает канал шага.
