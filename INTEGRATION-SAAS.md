...
Провайдеры каналов - ПЛАТФОРМЕННЫЕ, ключи держим мы: email (Resend),
SMS/Viber/WhatsApp (DecisionTelecom), Telegram (бот). Клиент ничего не
подключает у провайдеров - его зона это КОНТАКТЫ юзеров и их СОГЛАСИЯ.

**Email** работает из коробки: адреса приходят из Stripe при склейке.
